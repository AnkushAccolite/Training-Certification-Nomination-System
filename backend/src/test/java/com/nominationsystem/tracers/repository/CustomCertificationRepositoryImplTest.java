...
public class CustomCertificationRepositoryImplTest {
    ...
    @BeforeEach
    public void setUp() {
        when(mongoTemplate.getCollection("Certification")).thenReturn(mongoCollection);
    ...
    public void testFindDistinctDomains() {
        ...
        List<String> result = customCertificationRepository.findDistinctDomains();
        assertEquals(expectedDomains, result);

        verify(mongoTemplate, times(1)).getCollection("Certification");
